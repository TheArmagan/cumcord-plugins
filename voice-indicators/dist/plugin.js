(function(){"use strict";const{findByProps:r}=cumcord.modules.webpack,{injectCSS:v}=cumcord.patcher,p=r("getGuild","getGuildCount"),f=r("getChannel","hasChannel"),d=r("getVoiceState","getUserVoiceChannelId"),g=r("transitionTo"),c={DANGER:"#eb3d47",SECONDARY:"#8a8e93",SUCCESS:"#3aa360"},l=[];function u(t){let e=d.getVoiceStateForUser(t);return e?e.isVoiceDeafened()?e.deaf?"guild-deaf":"self-deaf":e.isVoiceMuted()?e.mute?"guild-mute":"self-mute":"voice":null}function h(t){let e=d.getVoiceStateForUser(t);if(!e)return null;let a=f.getChannel(e.channelId),i=p.getGuild(a.guild_id),C=S(`
    <div class="vi--container" data-vi-status="${u(t)}">
      <div class="vi--icon-container">
        ${e.isVoiceDeafened()?`
      <svg class="vi--icon vi--deaf-icon" width="24" height="24" viewBox="0 0 24 24" style="color:${c[e.deaf?"DANGER":"SECONDARY"]};">
    ...
    `:e.isVoiceMuted()?`
      <svg class="vi--icon vi--mute-icon" width="24" height="24" viewBox="0 0 24 24" style="color:${c[e.mute?"DANGER":"SECONDARY"]};">
    ...
    `:`
      <svg class="vi--icon vi--voice-icon" width="60" height="61" viewBox="0 0 60 61" style="color:${c.SECONDARY};">
  ...
  `);return C.querySelector(".vi--icon-container").addEventListener("click",y=>{y.preventDefault(),g.transitionTo(`/channels/${i.id}/${a.id}`)}),C}function s(t,e){let a=t.querySelector("[data-vi-status]"),i=u(e);a?i?a.getAttribute("data-vi-status")!=i&&a.replaceWith(h(e)):a.remove():i&&t.appendChild(h(e))}let n=!1;async function m(){n||(n=!0,l.push(v(`
    .vi--icon-container {
      display: flex;
      align-items: center;
  ...
  `)),l.push((()=>{let t=setInterval(()=>{[...document.querySelectorAll('[class*="member-"][class*="container-"] [class*="avatarStack-"] [src*="avatars"], [class*="channel-"][class*="container-"] [class*="avatarStack-"] [src*="avatars"]')].forEach(e=>{let a=e.getAttribute("src").split("/")[4],i=o(e,6).querySelector('[class*="nameAndDecorators-"]');s(i,a)}),[...document.querySelectorAll('[class*="message-"][class*="wrapper-"] [class*="contents-"] [src*="avatars"]')].forEach(e=>{let a=e.getAttribute("src").split("/")[4],i=o(e,1).querySelector('h2[class*="header-"]');s(i,a)}),[...document.querySelectorAll('[class*="peopleListItem-"] [class*="listItemContents-"] [class*="avatarStack-"] [src*="avatars"]')].forEach(e=>{let a=e.getAttribute("src").split("/")[4],i=o(e,6).querySelector('[class*="discordTag-"]');s(i,a)})},500);return()=>{clearInterval(t)}})()))}async function L(){!n||(n=!1,l.forEach(t=>t()),document.querySelectorAll(".vi--container").forEach(t=>t.remove()))}function S(t){let e=document.createElement("div");return e.innerHTML=t,e.children[0]}function o(t,e){for(let a=0;a<e;a++)t=t.parentElement;return t}var V={onLoad(){m()},onUnload(){L()}};return V})();
